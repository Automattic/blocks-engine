...
- Product-neutral HTML parity gaps are covered as reusable primitives: empty decorative visual placeholders become empty core groups when they carry presentation/structure signals, presentation-only media wrappers transfer representable class identity onto a single native `core/image` when DOM, layout, runtime, identity, interaction, box-model, and selector equivalence prove the wrapper boundary is unnecessary, definition and nested lists keep their visible hierarchy, local forms and scripts produce scoped fallback diagnostics, resized SVG image references serialize through core image markup, and code-window labels from captions or data attributes are preserved as visible text next to the code block.
- A captioned figure whose image sits in an inner paragraph (`<figure><p><img></p><figcaption>`) becomes native `core/image` with a caption. The inner paragraph is a transparent media carrier, not a spacing donor, and is not reintroduced. In-flow image margins that only existed because the image was not a `figure > *` child are a source-nesting artifact; they are not copied onto the image block. Captioned-figure pages are excluded from strict document-height parity (#2043). The figure element's own representable presentation (classes, authored figure margin) still transfers, as in `html-figure-quote-media.json`.
