...
- Generic conversion report projections in `source_reports.conversion_report` for fallback diagnostics, source/selector summaries, asset references, navigation candidates, visual presentation signals, and metrics. These reports are views over transformer-owned data, not product compatibility envelopes.
