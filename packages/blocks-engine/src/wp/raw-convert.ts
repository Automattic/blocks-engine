...
type Serialize = (blocks: unknown[]) => string;
...
function preprocess(html: string): { html: string; spacers: string[] } {
 ...
 */
export function rawConvert(html: string): { html: string | null; wpHtmlResidue: number } {
  bootstrap();
  if (!html || !html.trim()) return { html: '', wpHtmlResidue: 0 };
  try {
    ...
    markup = markup.replace(
      ...
      (_: string, i: string) => spacerBlock(spacers[+i]),
    );
    ...
    console.error('[rawConvert] convert failed:', e && (e as { message?: unknown }).message);
    return { html: '', wpHtmlResidue: Infinity };
  }
}
