...
import { isWpMediaUrl, pickLeadImage } from './native-media.js';
...
export function renderTextBand(section: SectionSpec, ctx: NativeRenderCtx): NativeRenderOut {
  ...
  if (lead) {
    parts.push(
      imageBlock(lead, out, `${section.interactionModel}#${section.sectionIndex}`, {
  ...
  if (extra.length >= 3) {
    const gallery = galleryBlock(extra, out);
    if (gallery) parts.push(gallery);
  } else {
    for (const image of extra.filter((candidate) => pickLeadImage([candidate]) === candidate)) {
      parts.push(
        imageBlock(image, out, `${section.interactionModel}#${section.sectionIndex}.extra`, {
          align: centerOf(section) ? 'center' : null,
          rounded: true,
        }),
      );
...
export function renderCover(section: SectionSpec, ctx: NativeRenderCtx): NativeRenderOut {
  const lead = pickLeadImage(section.images);
  if (!lead || !isWpMediaUrl(lead.url)) return renderTextBand(section, ctx);
  const out = emptyNativeRenderOut();
...
export function renderMediaText(section: SectionSpec, flip: boolean, ctx: NativeRenderCtx): NativeRenderOut {
  ...
  const imageMarkup = imageBlock(lead, out, `media-text#${section.sectionIndex}`, { rounded: true });
  ...
  if (extra.length >= 3) {
    ...
    for (const image of extra.filter((candidate) => pickLeadImage([candidate]) === candidate)) {
      blocks.push(imageBlock(image, out, `media-text#${section.sectionIndex}.extra`, { align: 'center', rounded: true }));
    }
  }
...
export function galleryBlock(
  ...
  const usable = images.filter((image) => isWpMediaUrl(image.url));
  ...
  const rowHeight = sortedHeights.length
    ...
    : 0;
  const figures = usable.map((image) => {
    out.assets.push(image.url);
    if (image.width && image.height && rowHeight) {
      ...
      return (
        ...
        `<figure class="wp-block-image size-large is-resized"><img src="${escapeHtml(image.url)}" alt="${escapeHtml(image.alt || '')}" style="width:${width}px;height:${rowHeight}px"/></figure>\n` +
        `<!-- /wp:image -->`
      );
    }
    return (
      `<!-- wp:image {"sizeSlug":"large","linkDestination":"none"} -->\n` +
      `<figure class="wp-block-image size-large"><img src="${escapeHtml(image.url)}" alt="${escapeHtml(image.alt || '')}"/></figure>\n` +
      `<!-- /wp:image -->`
    );
  });
