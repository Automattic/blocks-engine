...
// returns false and stays on the verbatim island render, where the source DOM +
// classes survive so the carried CSS binds 1:1.
//
// Ported from data-liberation-agent (src/lib/replicate/semantic-html.ts). The
// unwrap list mirrors the rawConvert preprocess unwrap (kept in sync by hand).
//
...
// (conservative: stays on the verbatim island rather than risking a
...
export function isSemanticHtml(html: string): boolean {
  ...
  while (changed) {
    ...
    $(UNWRAP_SELECTOR).each((_, el) => {
      const $el = $(el);
      // defensive: spacers don't actually match UNWRAP_SELECTOR
