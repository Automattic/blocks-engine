...
final class WordPressSitePlanResolver
{
    ...
    public function resolve(array $plan, array $context): array
    {
        ...
        foreach ($plan['writes'] as &$write) if ('utf8' === $write['payload']['encoding']) { $write['canonical_payload'] = $write['payload']['data']; $write['canonical_payload_hash'] = WordPressSitePlan::contentHash($write['canonical_payload']); $write['payload']['data'] = self::resolvePayload($write['canonical_payload'], self::writeReferences($plan['reference_tokens'], $write['target_path'])); $write['payload_hash'] = WordPressSitePlan::contentHash($write['payload']['data']); }
        unset($write);
        ...
        $plan['resolution'] = array('schema' => self::RESOLUTION_SCHEMA, 'theme_uri' => $themeUri, 'runtime_capabilities' => $capabilities, 'asset_publication_references' => self::publicationReferences($plan['runtime_declarations'], $plan['reference_tokens'], $plan['writes']), 'unsupported_optional_capabilities' => $unsupportedOptional);
        WordPressSitePlan::assertValid($plan);
        return $plan;
    }
    ...
    /** @param array<int,array<string,mixed>> $tokens @return array<string,string> */
    public static function writeReferences(array $tokens, string $origin): array
    {
        $references = array();
