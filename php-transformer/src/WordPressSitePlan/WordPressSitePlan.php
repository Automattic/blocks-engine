...
final class WordPressSitePlan
{
    ...
    private static function assertDocumentMetadata(array $metadata, array $tokens): void { if(!is_string($metadata['title']??null)||!is_array($metadata['meta']??null)||!is_array($metadata['links']??null)||!is_array($metadata['scripts']??null))throw new InvalidArgumentException('WordPress site plan document metadata is structurally invalid.');foreach($metadata['meta'] as $row)if(!is_array($row)||array()=== $row||array_diff(array_keys($row),array('charset','name','property','http_equiv','content')))throw new InvalidArgumentException('WordPress site plan meta declaration is invalid.');foreach($metadata['links'] as $row){if(!is_array($row)||!in_array($row['placement']??null,array('head','body'),true)||(!is_string($row['asset_reference']??null)&&!is_string($row['url']??null)))throw new InvalidArgumentException('WordPress site plan link declaration is invalid.');if(is_string($row['asset_reference']??null))self::assertTokens($row['asset_reference'],$tokens);}foreach($metadata['scripts'] as $row){if(!is_array($row)||!in_array($row['placement']??null,array('head','body'),true)||!in_array($row['load']??null,array('blocking','defer','async'),true)||!is_bool($row['defer']??null)||!is_bool($row['async']??null)||(('async'===($row['load']??null))!==$row['async'])||(('defer'===($row['load']??null))!==$row['defer'])||(!is_string($row['asset_reference']??null)&&!is_string($row['url']??null)&&'inline'!==($row['source_kind']??null)))throw new InvalidArgumentException('WordPress site plan script declaration is invalid.');if(is_string($row['asset_reference']??null))self::assertTokens($row['asset_reference'],$tokens);}}
