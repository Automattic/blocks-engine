...
// src/runtime/region-effect-analyzer.ts
var region_effect_analyzer_exports = {};
__export(region_effect_analyzer_exports, {
  REGION_EFFECT_ANALYZER_BUNDLE: () => REGION_EFFECT_ANALYZER_BUNDLE
});
module.exports = __toCommonJS(region_effect_analyzer_exports);
...
function analyzeRuntimeRegionEffects(source) {
  ...
  let program;
  try {
    program = parse3(source, { ecmaVersion: "latest", sourceType: "script" });
  } catch {
    ...
    if (statement.type === "VariableDeclaration") {
      for (const declaration of statement.declarations) {
        if (declaration.id.type === "Identifier")
          declared.set(declaration.id.name, (declared.get(declaration.id.name) ?? 0) + 1);
      }
    }
  }
  return {
    ...
    sourceHash,
    units: program.body.map((statement, index) => unitFor(statement, index, source, sourceHash, declared))
  };
}
function unitFor(statement, index, source, sourceHash, declared) {
  ...
  walk(statement, (node) => {
    ...
    if ((name === "querySelector" || name === "querySelectorAll" || name === "getElementById") && node.arguments.length) {
      ...
      if (argument.type !== "Literal" || typeof argument.value !== "string")
        ...
        targets.add(name === "getElementById" ? `#${argument.value}` : argument.value);
    }
    ...
    if (["add", "remove", "toggle", "replace"].includes(name) && node.callee.object?.type === "MemberExpression" && node.callee.object.property?.name === "classList")
      mutations.add("class");
  ...
  const reason = dynamicSelector ? "dynamic_selector" : shared ? "shared_state" : !recognized || !targets.size ? "unrecognized_pattern" : void 0;
  const unit = {
...
// src/runtime/region-effect-analyzer.ts
var REGION_EFFECT_ANALYZER_BUNDLE = "blocks-engine/runtime-region-effect-analyzer/v1";
async function main() {
