...
$publicationFace = '@font-face{font-family:Example;src:url(font.woff2)}';
...
$publicationInput = array('css' => array(array('source_path' => 'assets/fonts.css', 'content_hash' => hash('sha256', $publicationCss), 'font_faces' => array($publicationFace))), 'fonts' => array(array('source_path' => 'assets/font.woff2', 'content_hash' => hash('sha256', base64_encode($publicationFont)))), 'font_faces' => array());
$publicationDeclaration = array('kind' => 'asset_publication', 'type' => 'asset', 'source_path' => 'assets/logo.svg', 'provenance' => array('source_path' => 'assets/logo.svg', 'source' => 'artifact'), 'destination' => array('capability' => 'asset_materialization', 'required' => true), 'source_role' => 'image', 'mime_type' => 'image/svg+xml', 'source_hash' => hash('sha256', $publicationSvg), 'expected_content_hash' => hash('sha256', $publicationContent), 'sanitization' => array('schema' => 'generic/svg-sanitization/v1', 'input_hash' => hash('sha256', $publicationSvg)), 'reference_targets' => array('assets/assets/fonts.css'), 'transformation' => array('kind' => 'svg_font_enrichment', 'css_source_paths' => array('assets/fonts.css'), 'font_source_paths' => array('assets/font.woff2'), 'font_faces' => array(), 'input_hash' => RuntimeDeclarations::hash($publicationInput), 'expected_content_hash' => hash('sha256', $publicationContent)));
...
$publicationResolved = (new WordPressSitePlanResolver())->resolve($publicationPlan, array('theme_uri' => 'https://example.test/theme'));
...
$undeclaredPublication = $publicationArtifact; $undeclaredPublication['runtime_declarations'][0]['reference_targets'] = array('missing/write.css'); $undeclaredPublicationResult = (new ArtifactCompiler())->compile($undeclaredPublication)->toArray(); $assert(isset($undeclaredPublicationResult['source_reports']['wordpress_site_plan_diagnostics']), 'Asset publication rejects undeclared destination replacement writes.');
$unsafePublication = $publicationArtifact; $unsafePublication['files']['assets/logo.svg'] = '<svg><script>alert(1)</script></svg>'; $unsafePublication['runtime_declarations'][0]['source_hash'] = hash('sha256', $unsafePublication['files']['assets/logo.svg']); $unsafePublication['runtime_declarations'][0]['sanitization']['input_hash'] = $unsafePublication['runtime_declarations'][0]['source_hash']; $unsafePublicationResult = (new ArtifactCompiler())->compile($unsafePublication)->toArray(); $assert(isset($unsafePublicationResult['source_reports']['wordpress_site_plan_diagnostics']), 'Unsafe SVG publication cannot produce a materialization plan.');
