...
$referenceResult = (new ArtifactCompiler())->compose($referencedShared, array_reverse($referencedReceipts), $pagePayloadsRemoved);
$receiptResult = $referenceResult->toArray();
...
$referenceCompactView = (new WordPressSitePlanView())->compact($referenceResult->toWordPressSitePlanView());
$assert(array() === $pagePayloadsRemoved->reads && $referenceCompactView === $referenceResult->toCompactWordPressSitePlanView() && 0 === ($referenceResult->metrics['html_document_transform_count'] ?? null) && 0 === ($referenceResult->metrics['normalization_count'] ?? null), 'Compact staged projection preserves the reference-backed result view exactly without terminal payload reads or losing terminal metrics.');
