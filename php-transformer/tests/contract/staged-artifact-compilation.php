...
$assert(str_contains($bootstrap, "add_action( 'enqueue_block_assets'") && str_contains($bootstrap, "if ( wp_should_load_block_editor_scripts_and_styles() ) return;") && !str_contains($bootstrap, "add_filter( 'wp_theme_json_data_theme'") && !str_contains($bootstrap, "blocks_engine_presentation_css") && !str_contains($bootstrap, "file_get_contents( get_theme_file_path( \$style['target_path'] ) )"), 'Canonical bootstrap uses WordPress iframe collection to isolate external presentation styles from editor chrome without reading, theme-JSON materializing, or inline-settings serializing stylesheet bodies.');
