...
$assert(str_contains($bootstrap, "empty( \$style['author_css'] )") && str_contains($bootstrap, "\$style['content_hash']") && str_contains($bootstrap, "\$style['media'] ?? 'all'") && str_contains($bootstrap, "get_theme_file_uri( \$style['target_path'] )") && str_contains($bootstrap, "'baseURL' => get_theme_file_uri( \$style['target_path'] )") && str_contains($bootstrap, "'isGlobalStyles' => false"), 'Canonical editor styles retain external engine-support delivery and route authored styles through Core editor settings with base-URL resolution.');
