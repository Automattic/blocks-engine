...
if ('provider_runtime' !== ($coverage['blocks']['core/social-links']['applicability'] ?? null) || 'implemented' !== ($coverage['blocks']['core/social-links']['implementation'] ?? null) || ! in_array('core/social-link', $coverage['supported_blocks'], true)) throw new RuntimeException('The dynamic social-link family must be reported as implemented without conflating provider runtime availability with transformer output.');
...
if ('version_gated' !== ($coverage['blocks']['core/tabs']['applicability'] ?? null) || '7.1' !== ($coverage['blocks']['core/tabs']['minimum_runtime'] ?? null) || 'implemented' !== ($coverage['blocks']['core/tabs']['implementation'] ?? null) || 'contract_tested' !== ($coverage['blocks']['core/tabs']['verification'] ?? null) || ! in_array('core/tab-panel', $coverage['supported_blocks'], true)) throw new RuntimeException('The transformer-owned Tabs family must retain its WordPress 7.1 gate and be reported as implemented and verified.');
if (array('core/paragraph', 'core/tabs') !== $coverage['runtime_available_blocks'] || array() !== $coverage['runtime_registered_blocks']) throw new RuntimeException('Runtime availability must be reported independently of capability classification and live registrations.');
if (115 !== count($coverage['bundled_snapshot_blocks'] ?? array()) || !in_array('core/verse', $coverage['bundled_snapshot_blocks'] ?? array(), true)) throw new RuntimeException('Bundled snapshot knowledge must be reported independently of runtime availability and transformer support.');
if (in_array('core/verse', $coverage['supported_blocks'], true)) throw new RuntimeException('Known but unimplemented snapshot blocks must not be reported as transformer output.');
