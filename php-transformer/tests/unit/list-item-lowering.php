...
$archive = (new HtmlTransformer())->transform(
    '<style>.index{display:grid;gap:1.5rem}.row{display:flex}.row-inner{display:flex;width:100%;align-items:flex-start}'
    . '.date{width:6rem;flex-shrink:0}.body{flex:1 1 0%}.title{margin:0}.dek{margin-top:.25rem;margin-bottom:.25rem}</style>'
    . '<ul class="index"><li class="row"><div class="row-inner"><span class="date">Jun 4</span>'
    . '<div class="body"><h3 class="title"><a href="/one">One</a></h3>'
    . '<div class="dek">Excerpt one</div><a href="/one">Read more</a></div></div></li>'
    . '<li class="row"><div class="row-inner"><span class="date">Jan 26</span>'
    . '<div class="body"><h3 class="title"><a href="/two">Two</a></h3>'
    . '<div class="dek">Excerpt two</div><a href="/two">Read more</a></div></div></li></ul>'
)->toArray();
$archiveMarkup = (string) ($archive['serialized_blocks'] ?? '');
...
if (0 !== ($boldSvgLabel['source_reports']['editability_report']['metrics']['structural_rich_text_attribute_count'] ?? -1) || !str_contains($boldSvgLabelMarkup, '<b><img src="assets/materialized-svg/') || !str_contains($boldSvgLabelMarkup, 'Krishishala AI</b></p>') || !str_contains($boldSvgLabelCss, '.bot p.blocks-engine-inline-layout-carrier > b{color:#176247}') || str_contains($boldSvgLabelMarkup, '<svg') || str_contains($boldSvgLabelMarkup, '<!-- wp:html') || 1 !== count(array_filter($boldSvgLabel['assets'] ?? array(), static fn (array $asset): bool => 'inline-svg' === ($asset['source'] ?? null))) || 'pass' !== ((new BlockValidityValidator())->validateBlocks($boldSvgLabel['blocks'] ?? array())['status'] ?? '')) throw new RuntimeException('Bold inline SVG labels materialize their passive artwork as native RichText images while preserving formatted source text and its author color rule.');
