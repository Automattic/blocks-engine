...
$assert(
    ...
    && str_contains($externalLayoutCss, ':root :where(.blocks-engine-css-owned-grid)>:where(:not(h1,h2,h3,h4,h5,h6)){margin-block-start:0;margin-block-end:0}')
...
$listStyleGroups = $findBlocks($listStyles['blocks'] ?? array(), 'core/group');
$listStyleCss = implode("\n", array_column($listStyles['assets'] ?? array(), 'content'));
$listStyleMarkup = (string) ($listStyles['serialized_blocks'] ?? '');
$assert(2 === count($findBlocks($listStyles['blocks'] ?? array(), 'core/list')) && 2 === count($listStyleItems) && 3 <= count($listStyleGroups) && ! str_contains($listStyleMarkup, '<!-- wp:html'), 'wrapped structural list content decomposes into editable native Groups while representable nested lists remain core/list');
$assert(str_contains($listStyleMarkup, 'tagName":"ol"') && 1 === substr_count($listStyleMarkup, 'tagName":"li"') && str_contains($listStyleMarkup, 'stage-copy blocks-engine-css-owned-layout') && str_contains($listStyleMarkup, 'blocks-engine-source-div-') && str_contains($listStyleMarkup, '>Maintenance detail</p>') && str_contains($listStyleMarkup, 'className":"chips"') && str_contains($listStyleMarkup, '>HTML</li>') && str_contains($listStyleMarkup, 'className":"check-list"') && str_contains($listStyleMarkup, '>Verified delivery</li>'), 'native structural groups retain source-tag provenance, nested list topology, and editable leaves');
$assert(str_contains($listStyleCss, 'blocks-engine-source-') && str_contains($listStyleCss, 'display:grid') && str_contains($listStyleCss, 'margin:.25rem 0 0') && str_contains($listStyleCss, 'position:relative') && str_contains($listStyleCss, 'padding:0 0 0 1.75rem') && str_contains($listStyleCss, '::before') && str_contains($listStyleCss, ':hover{color:#123456}'), 'projected author CSS continues to address native structural descendants, nested list leaves, and pseudo-elements');
$assert(isset($listStyles['source_reports']['wordpress_site_plan']) && ! str_contains((string) ($listStyles['source_reports']['wordpress_site_plan']['pages'][0]['canonical_block_markup'] ?? ''), '<!-- wp:html') && str_contains((string) ($listStyles['source_reports']['wordpress_site_plan']['pages'][0]['canonical_block_markup'] ?? ''), 'tagName":"ol"'), 'external structural-list styling survives artifact compilation through native block decomposition');
