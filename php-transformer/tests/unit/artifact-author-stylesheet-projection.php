...
$listStyleFallbacks = $findBlocks($listStyles['blocks'] ?? array(), 'core/html');
$listStyleCss = implode("\n", array_column($listStyles['assets'] ?? array(), 'content'));
$listStyleMarkup = (string) ($listStyles['serialized_blocks'] ?? '');
$assert(0 === count($findBlocks($listStyles['blocks'] ?? array(), 'core/list')) && 0 === count($listStyleItems) && 1 === count($listStyleFallbacks) && str_contains($listStyleMarkup, '<!-- wp:html '), 'wrapped structural list content uses one coherent explicit fallback instead of opaque list-item RichText');
$outerContent = (string) ($listStyleFallbacks[0]['attrs']['content'] ?? '');
$assert(str_contains($outerContent, 'stage-copy blocks-engine-source-div-') && str_contains($outerContent, 'blocks-engine-preserved-html-') && str_contains($outerContent, '>Build source<p class="blocks-engine-source-p-') && str_contains($outerContent, '>Maintenance detail</p>') && str_contains($outerContent, 'class="chips ') && str_contains($outerContent, '>HTML</li>') && str_contains($outerContent, 'class="check-list ') && str_contains($outerContent, '>Verified delivery</li>'), 'structural list fallback retains source-tag provenance and nested topology for projected author CSS');
$assert(str_contains($listStyleCss, ':where(.blocks-engine-preserved-html-') && str_contains($listStyleCss, 'display:grid') && str_contains($listStyleCss, 'margin:.25rem 0 0') && str_contains($listStyleCss, 'position:relative') && str_contains($listStyleCss, 'padding:0 0 0 1.75rem') && str_contains($listStyleCss, '.check-list li::before') && str_contains($listStyleCss, ':hover{color:#123456}'), 'projected author CSS continues to address retained rich descendants, nested list leaves, and pseudo-elements');
$assert(isset($listStyles['source_reports']['wordpress_site_plan']) && str_contains((string) ($listStyles['source_reports']['wordpress_site_plan']['pages'][0]['canonical_block_markup'] ?? ''), '<!-- wp:html') && ! str_contains((string) ($listStyles['source_reports']['wordpress_site_plan']['pages'][0]['canonical_block_markup'] ?? ''), '<!-- wp:list-item'), 'external list-item styling survives artifact compilation through explicit structural-list fallback');
