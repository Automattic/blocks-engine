...
$innerSurfaceMargin = $transform('<style>.button-surface{display:inline-block;min-width:170px;padding:22px 26px;background:#fff;color:#000}.button-surface:after{content:" \\2192";display:inline-block;margin-left:12px}</style><a class="button-shell" href="/go"><span class="button-surface">Go</span></a>');
$innerSurfaceMarginCss = $css($innerSurfaceMargin);
$assert('core/buttons' === ($innerSurfaceMargin['blocks'][0]['blockName'] ?? '') && str_contains($innerSurfaceMarginCss, '> :where(.wp-block-button__link):after{content:" \\2192";display:inline-block;margin-left:12px}') && ! preg_match('/\.wp-block-buttons\):after\{[^}]*margin-left:12px/', $innerSurfaceMarginCss) && 'pass' === ($innerSurfaceMargin['source_reports']['wp_block_validity']['status'] ?? ''), 'inner-surface generated-content margins remain on the native button link pseudo-element rather than the core/buttons wrapper');
...
$assert(str_contains($structuredLogoMarkup, '<!-- wp:blocks-engine/author-layout') && str_contains($structuredLogoMarkup, '<a href="/" class="wp-block-blocks-engine-author-layout logo" aria-label="Home">') && str_contains($structuredLogoMarkup, '<span class="logo-mark" aria-hidden="true"') && ! str_contains($structuredLogoMarkup, '<!-- wp:button') && ! str_contains($structuredLogoMarkup, '<!-- wp:html') && str_contains($structuredLogoCss, '.logo{display:inline-flex;align-items:center;gap:.6rem;text-decoration:none}') && str_contains($structuredLogoCss, '.logo:hover{color:red}') && 'pass' === ($structuredLogo['source_reports']['wp_block_validity']['status'] ?? ''), 'structured logo anchors preserve their authored anchor topology and CSS without inventing button chrome');

$headerTopology = $transform('<style>.site-header{display:flex;align-items:center}.site-title{font-family:Arial,sans-serif;color:#000}.site-title .wordmark{font-weight:700}.main-nav{margin-left:20px}</style><header class="site-header"><a class="site-title" href="/"><span class="wordmark">Northwind</span></a><nav class="main-nav"><ul><li><a href="/about">About</a></li></ul></nav></header>');
$headerTopologyMarkup = (string) ($headerTopology['serialized_blocks'] ?? '');
$headerTopologyCss = $css($headerTopology);
$headerTopologyBlocks = $headerTopology['blocks'][0]['innerBlocks'] ?? array();
$assert('blocks-engine/author-layout' === ($headerTopologyBlocks[0]['blockName'] ?? '') && 'core/navigation' === ($headerTopologyBlocks[1]['blockName'] ?? '') && str_contains($headerTopologyMarkup, '<a href="/" class="wp-block-blocks-engine-author-layout site-title"><span class="wordmark"') && str_contains($headerTopologyCss, 'font-family:Arial,sans-serif') && str_contains($headerTopologyCss, 'color:#000') && ! str_contains($headerTopologyMarkup, '<!-- wp:button'), 'header topology retains an authored site-title anchor and native navigation as sibling flex items');
...
$assert(str_contains($plainSvgLogoMarkup, '<!-- wp:blocks-engine/author-layout') && str_contains($plainSvgLogoMarkup, '<img src="assets/materialized-svg/') && str_contains($plainSvgLogoMarkup, 'Relay Atlas') && str_contains($plainSvgLogoCss, 'margin-right:auto') && ! str_contains($plainSvgLogoMarkup, '<!-- wp:html') && 'pass' === ($plainSvgLogo['source_reports']['wp_block_validity']['status'] ?? ''), 'linked text logos preserve authored anchor topology, inline artwork, and flex-item spacing');
...
$assert(str_contains($gridRichTextMarkup, '<div class="wp-block-blocks-engine-author-layout artifact-card">') && ! str_contains($gridRichTextMarkup, '<!-- wp:group') && str_contains($gridRichTextMarkup, 'class="wp-block-blocks-engine-author-layout card-label"') && str_contains($gridRichTextMarkup, '>Input</span>') && str_contains($gridRichTextCss, 'grid-template-columns:1fr auto') && str_contains($gridRichTextCss, 'grid-column:1 / -1') && 'pass' === ($gridRichText['source_reports']['wp_block_validity']['status'] ?? ''), 'grid cards retain selector-addressable direct children in a valid author-layout save shape');
