...
$assert(str_contains($iconOnlyMarkup, '<button type="button" class="wp-block-button__link') && str_contains($iconOnlyMarkup, 'title="Open filters"') && str_contains($iconOnlyMarkup, '<img src="assets/materialized-svg/') && ! str_contains($iconOnlyMarkup, '>Open filters</button>') && ! str_contains($iconOnlyMarkup, 'aria-label=') && str_contains($iconOnlyCss, '{width:42px !important;height:42px !important}') && str_contains($iconOnlyCss, '> :where(.wp-block-button__link){width:42px') && str_contains($iconOnlyCss, ':hover{background:#111}') && str_contains($iconOnlyCss, ':focus{outline:3px solid #d14}') && 1 === count(array_filter($iconOnlyButton['assets'] ?? array(), static fn (array $asset): bool => 'inline-svg' === ($asset['source'] ?? ''))) && 'pass' === ($iconOnlyButton['source_reports']['wp_block_validity']['status'] ?? ''), 'direct icon-only buttons retain sanitized SVG artwork, a core-valid accessible title, wrapper geometry, and link-projected chrome without synthesized visible text');
...
$assert(1 === substr_count($listInlineMarkup, '<!-- wp:html ') && 3 <= substr_count($listInlineMarkup, 'data-blocks-engine-richtext-marker=') && str_contains($listInlineCss, ':where(.blocks-engine-preserved-html-') && str_contains($listInlineMarkup, 'class="blocks-engine-source-li-') && ! str_contains($listInlineCss, 'blocks-engine-semantic-') && str_contains($listInlineCss, 'display:grid;place-items:center;width:30px;height:30px;border-radius:50%;background:#c9f27b') && str_contains($listInlineMarkup, '>1</span>') && str_contains($listInlineMarkup, '>2</span>') && str_contains($listInlineMarkup, '>N</span>') && str_contains($listInlineMarkup, '>Nested</div>') && 'pass' === ($listInlineLeaves['source_reports']['wp_block_validity']['status'] ?? ''), 'structural list fallback retains source selector markers and nested list topology without hiding block markup in RichText');
...
$assert(str_contains($coexistingInlineFlowsMarkup, '<p class="blocks-engine-inline-layout-carrier"><strong>index.html</strong></p>') && str_contains($coexistingInlineFlowsCss, '.fallback-card > p.blocks-engine-inline-layout-carrier > strong{display:block}') && str_contains($coexistingInlineFlowsMarkup, 'data-blocks-engine-richtext-marker=') && str_contains($coexistingInlineFlowsMarkup, 'class="blocks-engine-source-li-') && str_contains($coexistingInlineFlowsCss, ':where(.blocks-engine-preserved-html-') && str_contains($coexistingInlineFlowsCss, 'place-items:center;width:30px;height:30px') && 'pass' === ($coexistingInlineFlows['source_reports']['wp_block_validity']['status'] ?? ''), 'standalone layout carriers coexist with explicit structural-list fallback and retained selector markers');
...
$assert(
    ...
    && str_contains($typographyOnlyStructuralMarkup, 'data-blocks-engine-richtext-marker=')
    && str_contains($typographyOnlyStructuralMarkup, 'class="blocks-engine-source-li-')
    && str_contains($typographyOnlyStructuralCss, ':where(.blocks-engine-preserved-html-')
    ...
    'direct Grid and Flex typography-only strong leaves retain valid standalone carriers while structural-list fallback preserves selector addressability and ordinary prose stays RichText'
);
