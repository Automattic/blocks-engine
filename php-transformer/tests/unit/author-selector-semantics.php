...
$assert(2 === count(array_unique($repeatedMarkers[0] ?? array())) && 2 === substr_count($repeatedCss, ':where(.blocks-engine-semantic-') && str_contains($repeatedMarkup, '<mark class="blocks-engine-richtext-') && str_contains($repeatedCss, 'mark.blocks-engine-richtext-'), 'repeated structural parents allocate unique source-path markers without leaking their box styles into an unrelated inline sibling');
...
$assert(str_contains($richTextPillMarkup, '<mark class="blocks-engine-richtext-') && str_contains($richTextPillCss, 'mark.blocks-engine-richtext-') && 'pass' === ($richTextPill['source_reports']['wp_block_validity']['status'] ?? ''), 'RichText-contained selector hooks survive through valid mark formatting and projected CSS');
...
$assert(str_contains($standaloneBadgeMarkup, 'className":"badge blocks-engine-semantic-') && str_contains($standaloneBadgeMarkup, 'padding-top:.25rem') && str_contains($standaloneBadgeMarkup, 'border-radius:999px') && str_contains($standaloneBadgeMarkup, 'background-color:#eee') && 'pass' === ($standaloneBadge['source_reports']['wp_block_validity']['status'] ?? ''), 'standalone styled badges retain independent native box geometry without HTML fallback');
...
$assert(1 === substr_count($inlineStatMarkup, '<!-- wp:paragraph') && str_contains($inlineStatMarkup, '>43</span><mark class="blocks-engine-richtext-') && str_contains($inlineStatMarkup, 'font-size:2rem') && str_contains($inlineStatMarkup, '>%</mark>'), 'non-structural inline metrics and suffixes remain in one styled RichText line');
...
$metadata = $transform('<style>.fact{display:grid}.fact-label{font-weight:600}.fact-value{font-family:monospace}</style><div class="fact"><span class="fact-label">Students</span><span class="fact-value">287</span></div>');
$metadataChildren = $metadata['blocks'][0]['innerBlocks'] ?? array();
