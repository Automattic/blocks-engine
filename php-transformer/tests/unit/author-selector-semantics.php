...
$assert(str_contains($gridRichTextMarkup, '<!-- wp:group {"className":"artifact-card blocks-engine-css-owned-layout"}') && ! str_contains($gridRichTextMarkup, 'wp-block-blocks-engine-author-layout') && str_contains($gridRichTextCss, 'grid-template-columns:1fr auto') && str_contains($gridRichTextCss, 'grid-column:1 / -1') && 'pass' === ($gridRichText['source_reports']['wp_block_validity']['status'] ?? ''), 'grid cards retain selector-addressable source CSS through core/group');
...
$assert('core/group' === ($authorGridBlock['blockName'] ?? '') && 5 === count($authorGridBlock['innerBlocks'] ?? array()) && str_contains($authorGridMarkup, '<section id="gallery" class="wp-block-group') && str_contains($authorGridMarkup, 'ex-row') && str_contains($authorGridMarkup, 'blocks-engine-css-owned-layout') && ! str_contains($authorGridMarkup, 'wp-block-blocks-engine-author-layout') && str_contains($css($authorGrid), 'margin-block-start:0!important'), 'author grids retain their semantic core/group save shape with scoped flow neutralization');
