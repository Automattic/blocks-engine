...
$assert(1 === substr_count($listInlineMarkup, '<!-- wp:html ') && 3 <= substr_count($listInlineMarkup, 'data-blocks-engine-richtext-marker=') && 3 === substr_count($listInlineCss, 'mark[style*="--blocks-engine-richtext-marker:') && str_contains($listInlineCss, '.maintenance-loop :where(.blocks-engine-source-li-') && str_contains($listInlineMarkup, 'class="blocks-engine-source-li-') && ! str_contains($listInlineCss, 'blocks-engine-semantic-') && str_contains($listInlineCss, 'display:grid;place-items:center;width:30px;height:30px;border-radius:50%;background:#c9f27b') && str_contains($listInlineMarkup, '>1</span><div>') && str_contains($listInlineMarkup, '>2</span><div>') && str_contains($listInlineMarkup, '>N</span><div>Nested</div>') && 'pass' === ($listInlineLeaves['source_reports']['wp_block_validity']['status'] ?? ''), 'structural list fallback retains source selector markers and nested list topology without hiding block markup in RichText');
