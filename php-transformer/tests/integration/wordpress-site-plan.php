...
$result = (new ArtifactCompiler())->compile(array('entrypoint' => 'index.html', 'files' => array(
    ...
    'assets/global.css' => 'body{color:#123456;background-color:#fefefe;background-image:url("logo.svg");font-family:Inter,sans-serif;font-size:18px;padding:24px}.global-presentation{display:block}',
...
$videoMarkup = (string) ((new HtmlTransformer())->transform('<media-host class="bg-video"><video class="player" src="hero.mp4" poster="hero.jpg" autoplay loop muted playsinline controls><track kind="captions" src="captions.vtt" srclang="en" label="English" default></video><div><img src="hero.jpg" alt=""></div></media-host>')->toArray()['serialized_blocks'] ?? '');
$videoId = wp_insert_post(array('post_type' => 'page', 'post_status' => 'draft', 'post_title' => 'Custom video', 'post_content' => serialize_blocks(parse_blocks($videoMarkup))), true);
if (is_wp_error($videoId)) throw new RuntimeException($videoId->get_error_message());
$pageIds['custom-video'] = $videoId;
$videoSaved = (string) get_post_field('post_content', $videoId);
$videoRendered = do_blocks($videoSaved);
$videoBlock = parse_blocks($videoSaved)[0] ?? array();
$nativeVideoBlock = $videoBlock['innerBlocks'][0] ?? array();
$assert('core/group' === ($videoBlock['blockName'] ?? null) && 'bg-video' === ($videoBlock['attrs']['className'] ?? null) && 'core/video' === ($nativeVideoBlock['blockName'] ?? null) && ! str_contains($videoSaved, '"playsInline":true') && array(array( 'kind' => 'captions', 'src' => 'captions.vtt', 'srcLang' => 'en', 'label' => 'English', 'default' => true )) === ($nativeVideoBlock['attrs']['tracks'] ?? null) && str_contains($videoRendered, '<video src="hero.mp4" poster="hero.jpg" controls="controls" autoplay="autoplay" loop="loop" muted="muted" playsinline="playsinline"><track kind="captions" src="captions.vtt" srclang="en" label="English" default="default"></video>'), 'WordPress persists a custom-host geometry wrapper around editable core/video markup with poster, tracks, and playback metadata.');
$flowNavigation = ( new HtmlTransformer() )->transform('<style>.flow-menu{display:block}</style><nav class="flow-menu"><ul><li><a href="/">Home</a></li></ul></nav>')->toArray();
$flowNavigationSaved = serialize_blocks(parse_blocks((string) ($flowNavigation['serialized_blocks'] ?? '')));
$flowNavigationRendered = do_blocks($flowNavigationSaved);
$assert(str_contains($flowNavigationSaved, '"layout":{"type":"default"}') && str_contains($flowNavigationRendered, 'is-layout-flow') && ! str_contains($flowNavigationRendered, 'is-layout-flex'), 'WordPress parses the serialized non-flex navigation layout and renders its flow class instead of core/navigation default flex.');
...
$accessibleLink = (new HtmlTransformer())->transform('<main><a class="lovable-badge button" href="https://lovable.dev" role="button" data-cta="badge" data-kind="partner" aria-describedby="badge-help" aria-label="Build with Lovable"><svg aria-hidden="true"><path d="M0 0h1v1z"/></svg><span>Créé 東京</span></a><a class="whatsapp-link" href="https://wa.me/15551234567" role="button" aria-label="Contactar por WhatsApp"><button type="button"><svg aria-hidden="true"><path d="M0 0h1v1z"/></svg></button></a></main>')->toArray();
$accessibleLinkMarkup = (string) ($accessibleLink['serialized_blocks'] ?? '');
$accessibleLinkId = wp_insert_post(array('post_type' => 'page', 'post_status' => 'draft', 'post_title' => 'Accessible links', 'post_content' => wp_slash($accessibleLinkMarkup)), true);
if (is_wp_error($accessibleLinkId)) throw new RuntimeException($accessibleLinkId->get_error_message());
$pageIds['accessible-links'] = $accessibleLinkId;
$accessibleLinkSaved = (string) get_post_field('post_content', $accessibleLinkId);
$accessibleLinkBlocks = parse_blocks($accessibleLinkSaved);
$accessibleLinkReloaded = serialize_blocks($accessibleLinkBlocks);
$assert('custom/accessible-link' === ($accessibleLinkBlocks[0]['innerBlocks'][0]['blockName'] ?? null) && 'custom/accessible-link' === ($accessibleLinkBlocks[0]['innerBlocks'][1]['blockName'] ?? null) && 'https://lovable.dev' === ($accessibleLinkBlocks[0]['innerBlocks'][0]['attrs']['href'] ?? null) && 'Build with Lovable' === ($accessibleLinkBlocks[0]['innerBlocks'][0]['attrs']['accessibleLabel'] ?? null) && 'badge' === ($accessibleLinkBlocks[0]['innerBlocks'][0]['attrs']['sourceAttributes']['data-cta'] ?? null) && 'partner' === ($accessibleLinkBlocks[0]['innerBlocks'][0]['attrs']['sourceAttributes']['data-kind'] ?? null) && 'badge-help' === ($accessibleLinkBlocks[0]['innerBlocks'][0]['attrs']['sourceAttributes']['aria-describedby'] ?? null) && str_starts_with((string) ($accessibleLinkBlocks[0]['innerBlocks'][0]['attrs']['content'] ?? ''), '<img') && str_contains((string) ($accessibleLinkBlocks[0]['innerBlocks'][0]['attrs']['content'] ?? ''), 'Créé 東京') && 'raw-source' === ($accessibleLinkBlocks[0]['innerBlocks'][1]['attrs']['contentMode'] ?? null) && str_contains((string) ($accessibleLinkBlocks[0]['innerBlocks'][1]['attrs']['content'] ?? ''), '<button type="button">') && str_contains($accessibleLinkReloaded, 'https://lovable.dev') && str_contains($accessibleLinkReloaded, 'https://wa.me/15551234567') && str_contains($accessibleLinkReloaded, 'data-cta="badge"') && !str_contains($accessibleLinkSaved, '<!-- wp:html'), 'WordPress parses, saves, reloads, and reparses the registered typed accessible-link companion for both retained Lovable and icon-only nested-button WhatsApp source shapes with source order, Unicode, URL, and semantic selector attributes.');
