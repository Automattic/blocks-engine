...
$result = (new ArtifactCompiler())->compile(array('entrypoint' => 'index.html', 'files' => array(
    'index.html' => '<!doctype html><html><head><script src="/assets/head.js?head=1#top"></script><script src="assets/defer.js" defer></script></head><body><a class="skip-link" href="#content">Skip to content</a><header id="site-chrome" class="site-chrome" style="border-top:3px solid #111"><p>Integration Header</p></header><main id="content"><img src="assets/logo.svg"><div style="display:flex"><a class="brand" href="/"><svg viewBox="0 0 10 10" aria-hidden="true"><circle cx="5" cy="5" r="4"/></svg><span>Brand</span></a></div><h1>Home</h1></main><footer class="site-footer"><p>Integration Footer</p></footer><script src="assets/async.js" async defer></script><script src="assets/module.js" type="module"></script><script src="assets/legacy.js" nomodule integrity="sha384-test" crossorigin="anonymous" referrerpolicy="no-referrer"></script><script src="https://cdn.example.test/external.js?build=1#run" async></script></body></html>',
...
$findAuthorLayout = static function (array $blocks) use (&$findAuthorLayout): ?array { foreach ($blocks as $block) { if ('blocks-engine/author-layout' === ($block['blockName'] ?? null) && 'rich-text' === ($block['attrs']['contentMode'] ?? null)) return $block; $found = $findAuthorLayout($block['innerBlocks'] ?? array()); if (is_array($found)) return $found; } return null; };
$authorLayout = $findAuthorLayout(parse_blocks($frontPage->post_content));
$themeSvgUrl = home_url('/wp-content/themes/' . $theme . '/assets/assets/materialized-svg/');
$assert(is_array($authorLayout) && str_contains((string) ($authorLayout['attrs']['content'] ?? ''), $themeSvgUrl) && str_contains($frontPage->post_content, $themeSvgUrl), 'Published WordPress block comments and their saved author-layout SVG markup resolve to the same theme asset URL.');
global $wp_query;
